                ...
                <div>
                    ...
                    {props.data?.fetchBoards.map((el,index)=>(
                        <S.DataListBox key={el._id}>
                            ...
                            <S.TitleBox id={el._id} onClick={props.GoToDetail}>{el.title}</S.TitleBox>
                            <S.WriterDate>{el.writer}</S.WriterDate>
                            <S.WriterDate>{el.createdAt.slice(0,10)}</S.WriterDate>
                        </S.DataListBox>
                    ))}
                </div>
            </S.WrapperList>
            <S.ListBottom>
                <S.CreateBtn onClick={props.CreateNew}><S.PencilIcon><FaPencilAlt /></S.PencilIcon>게시물 등록하기</S.CreateBtn>
            </S.ListBottom>
        </S.Wrapper>
    </S.NewBody>
    )
